// Bambu Bridge — BambuNetworkingPluginHandle implementation.
//
// dlopen+symbol-resolve+agent-lifecycle for the proprietary
// `bambu_networking` plugin. Extracted from CloudInventory in phase 6 so
// CloudUplink can share the same agent (the plugin only supports one
// agent per process).
//
// Symbols are the C++-by-value variants documented in the upstream
// NetworkAgent.hpp; using `const char*` instead would compile cleanly
// and corrupt the stack at runtime. Be careful when adding to the table.
...
// ---- ABI typedefs --------------------------------------------------------
// Mirror NetworkAgent.hpp upstream — see comment block in header for why
// these MUST use std::string / std::vector / std::function by value.
extern "C++" {
    ...
    // Cloud-side init that BambuStudio's NetworkAgent calls — without
    // these the plugin's HTTP REST requests miss the X-BBL-* fingerprint
    // (Bambu's cloud rejects on missing client identification), TLS to
    // the cloud has no trusted root (set_cert_file points at the bundled
    // base64-encoded Bambu cloud cert), and the realtime MQTT-to-cloud
    // session never comes up at all. Signatures mirror NetworkAgent.hpp
    // lines 16 / 31 / 77.
    ...
    // LAN-side exports — mirror NetworkAgent.hpp lines 28-29, 40-41, 66.
    // The plugin uses the same OnMessageFn signature (dev_id, msg) for
    // both cloud and local message callbacks; OnLocalConnectedFn carries
    // an int status plus the same (dev_id, msg) tail.
    ...
    // ---- Cloud upload via start_send_gcode_to_sdcard --------------------
    //
    // Mirror of `struct PrintParams` from upstream
    // `src/slic3r/Utils/bambu_networking.hpp`. The proprietary plugin
    // takes this BY VALUE, so the member order, types, and sizes must
    // match the upstream layout byte-for-byte. Defaults are unused at
    // the ABI boundary (the plugin only reads members our caller sets)
    // but we mirror them for documentation.
    //
    // Last verified against:
    //   ~/BambuStudio/src/slic3r/Utils/bambu_networking.hpp `struct PrintParams`
    //   (2026-05-11 — clean origin/master @ 3e96c7e07)
    ...
    // get_camera_url — see header for the full callback-style contract.
    // Mirrors MediaPlayCtrl.cpp's usage: the plugin returns int rc
    // synchronously and fires the callback with the resolved URL.
...
struct BambuNetworkingPluginHandle::Impl {
    ...
    // The std::function objects we hand to the plugin MUST outlive the
    // registration. The plugin stores them internally and invokes them
    // on its own threads; if we let them go out of scope between
    // `set_on_message_fn` and the next callback, we get a crash.
    ...
    // Per-dev_id message receivers. Locked by `mu`. The dispatcher runs
    // on the plugin's worker thread; the receiver itself runs in that
    // same context (cheap, just enqueues into LanUplink-style queues).
    std::mutex                                                                  mu;
    ...
    // Flipped by the OnServerConnected callback.
    std::atomic<bool> server_connected{false};
    // Flipped by the OnLocalConnected callback.
    std::atomic<bool> local_connected{false};

    // Tests flip this via set_agent_ready_for_test to bypass dlopen.
    ...
    bool load_plugin() {
        ...
        // create_agent is the absolute floor; without it nothing else
        // makes sense. The rest are optional — the handle degrades to
        // "feature unsupported" returns for missing entry points.
        return create_agent != nullptr;
    }

    bool ensure_started(BambuNetworkingPluginHandle* self) {
        if (!load_plugin()) return false;
        if (!agent) {
        ...
        // Order matches GUI_App::on_init_network exactly:
        //   set_config_dir → init_log → set_cert_file →
        //   set_extra_http_header → callbacks → set_country_code →
        //   start → (later) connect_server.
        ...
        // the plugin's connect path. Keep the std::function objects
        // alive in Impl.
        ...
        if (set_on_server_conn_fn) {
            auto* impl_ptr = this;
            on_server_conn_cb = [impl_ptr](int rc, int /*reason*/) {
                // Plugin's convention: rc == 0 means "connected".
                impl_ptr->server_connected.store(rc == 0);
            };
            set_on_server_conn_fn(agent, on_server_conn_cb);
        }

        // LAN-side callbacks. The plugin fires OnLocalMessageFn with the
        // same (dev_id, payload) shape as the cloud OnMessageFn — we
        // dispatch to a separate per-dev_id receiver map so cloud and
        // LAN traffic don't collide. OnLocalConnectedFn carries the int
        // status the plugin reports for the LAN MQTT session; rc == 0
        // means up.
        ...
        if (set_on_local_conn_fn) {
            on_local_conn_cb = [self](int rc, std::string /*dev_id*/,
                                      std::string /*msg*/) {
        ...
        // already reports is_user_login() == true synchronously when
        // cached tokens exist.
        if (connect_server) {
...
bool BambuNetworkingPluginHandle::is_server_connected() const {
    if (m_impl->ready_override.load()) {
        // Mock path: trust the recorded flag the test flipped.
        return m_impl->server_connected.load();
    }
    if (!m_impl->agent) return false;
    // Prefer the callback-driven flag (cheap, no plugin re-entrance).
...
int BambuNetworkingPluginHandle::publish_to_device(const std::string& dev_id,
    ...
    // The proprietary signature is (dev_id, json_str, qos, flag). `flag`
    // is reserved; upstream always passes 0.
...
void BambuNetworkingPluginHandle::dispatch_message(const std::string& dev_id,
    ...
    if (!cb) return;
    // The cloud broker only ever pushes one topic per dev_id (the
    // `device/<dev_id>/report` channel — see CloudMqttSession's
    // subscribe path in the host project). Synthesise it here so the
    // downstream slicer sees the same topic shape it would from a real
    // LAN printer.
    std::string topic = "device/" + dev_id + "/report";
    std::vector<uint8_t> bytes(payload.begin(), payload.end());
    cb(std::move(topic), std::move(bytes), /*qos=*/0);
}

// ---- LAN-side pass-through implementations -------------------------------
...
int BambuNetworkingPluginHandle::disconnect_printer() {
    ...
    // The plugin only fires OnLocalConnectedFn on connection-state
    // transitions, not on disconnect_printer() calls themselves. Flip
    // the flag locally so is_local_connected() reflects the requested
    // state immediately. If the plugin's callback later fires with a
    // contradictory rc the cb path will overwrite this.
...
void BambuNetworkingPluginHandle::dispatch_local_message(const std::string& dev_id,
    ...
    if (!cb) return;
    // Real Bambu LAN MQTT only ever pushes the `device/<dev_id>/report`
    // topic to slicers; synthesise it here so the downstream slicer sees
    // the same shape it would from a direct LAN connection.
    std::string topic = "device/" + dev_id + "/report";
    std::vector<uint8_t> bytes(payload.begin(), payload.end());
    cb(std::move(topic), std::move(bytes), /*qos=*/0);
}
...
void BambuNetworkingPluginHandle::dispatch_local_connected(bool connected) {
    ...
    if (cb) cb(connected ? 0 : -1, /*reason*/0);
}
...
int BambuNetworkingPluginHandle::get_camera_url(const std::string& dev_id,
    ...
    // Plugin invokes the callback asynchronously — sometimes inline before
    // returning, sometimes from a worker thread after a cloud round-trip.
    // Synchronise on a condvar with the caller's timeout. The callback
    // captures a shared state object so it stays live even if the plugin
    // fires the callback after our timeout has already elapsed (which the
