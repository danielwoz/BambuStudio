...
wxString GUI_App::current_language_code_safe() const
{
	...
	// Bridge-only / very-early startup may not have constructed wxLocale yet
	// (we skip load_language to avoid wx i18n init). Default to en_US in that
	// case so get_extra_header() / X-BBL-Language stays correct.
	if (!this->m_wxLocale) return "en_US";
	wxString language_code = this->current_language_code().BeforeFirst('_');
